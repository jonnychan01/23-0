...
export function RosterPanel({ roster, round, positionCounts, onMovePlayer }: Props) {
  ...
  const allDecades = Array.from(new Set(roster.map(p => p.decade))).sort();

  const filteredRoster = roster.filter(p => {
    if (filterPos !== "ALL" && p.position !== filterPos) return false;
    if (filterClub !== "ALL" && p.club !== filterClub) return false;
    if (filterDecade !== "ALL" && p.decade !== filterDecade) return false;
    return true;
  });
  ...
  const selectStyle = "text-xs border border-slate-200 rounded-lg px-2 py-1 bg-white text-slate-600 focus:outline-none focus:ring-1 focus:ring-blue-300 cursor-pointer";
    ...
    <div className="flex flex-col h-full bg-white w-full overflow-hidden">
      ...
      {roster.length > 0 && (
        <div className="hidden lg:flex shrink-0 items-center gap-2 px-3 py-2 border-b border-slate-200 bg-white flex-wrap">
          ...
          <select className={selectStyle} value={filterClub} onChange={e => setFilterClub(e.target.value)}>
            <option value="ALL">All clubs</option>
            {allClubs.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
          <select className={selectStyle} value={filterDecade} onChange={e => setFilterDecade(e.target.value)}>
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-slate-100 min-h-0">
        {roster.length === 0 ? (
          ...
          <p className="text-xs text-slate-400 text-center py-6">No players match the filters</p>
        ) : (
